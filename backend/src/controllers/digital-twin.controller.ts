...
export const digitalTwinController = {
     ...
     */
    async getMachineState(req: Request, res: Response): Promise<void> {
        try {
            const { machineId } = req.params;

            const state = await digitalTwinService.getMachineState(machineId);
    ...
    async simulateScenario(req: Request, res: Response): Promise<void> {
        try {
            ...
            const result = await digitalTwinService.simulateScenario(machineId, scenario, duration);
     ...
     */
    async testAllScenarios(req: Request, res: Response): Promise<void> {
        try {
            const { machineId } = req.params;

            const results = await digitalTwinService.testAllScenarios(machineId);
     ...
     */
    async getRUL(req: Request, res: Response): Promise<void> {
        try {
            const { machineId } = req.params;
     ...
     */
    async predictFailure(req: Request, res: Response): Promise<void> {
        try {
            const { machineId } = req.params;
        ...
        try {
            ...
            const twins = await Promise.all(
                machines.map(async (machine: any) => {
                    const state = await digitalTwinService.getMachineState(machine.id);
                    return {
                        id: machine.id,
                        name: machine.name,
                        type: machine.type,
     ...
     */
    async simulateProduction(req: Request, res: Response): Promise<void> {
        try {
            const { machineId } = req.params;
            ...
            const input: ProductionSimulationInput = {
                machineId,
                operationHours,
                targetProduction,
            ...
            const result: EnergyOptimizationResult = await digitalTwinService.simulateProduction(input);
     ...
     */
    async getProductionBaseline(req: Request, res: Response): Promise<void> {
        try {
            const { machineId } = req.params;

            const baseline = await digitalTwinService.getProductionBaseline(machineId);
