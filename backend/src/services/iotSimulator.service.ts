...
const MACHINE_PROFILES: Record<string, { voltage: number; current: number; power: number; temp: number; vibration: number; productionBase: number }> = {
...
const machineAnomalyStates: Record<string, {
    heatTrendingUp: number; // consecutive heat ticks 
    vibrationTrendingUp: number; // consecutive vibration ticks
}> = {};
 ...
 */
function addNoise(value: number, percent = 0.05): number {
 ...
 */
function maybeSpikeAnomaly(value: number, chance = 0.03): number {
 ...
 */
export function startIoTSimulation(broadcast: (payload: object) => void) {
        ...
        try {
            ...
            const readings = machines.map((machine) => {
                ...
                if (Math.random() < 0.02 || state.heatTrendingUp > 0) {
                    if (state.heatTrendingUp < 50) state.heatTrendingUp++; // Cap at severe overheating
                ...
                if (Math.random() < 0.02 || state.vibrationTrendingUp > 0) {
                    if (state.vibrationTrendingUp < 100) state.vibrationTrendingUp++;
...
export function stopIoTSimulation() {
