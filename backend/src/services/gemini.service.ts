...
export interface AIAutonomousDecision {
    ...
    actionTaken: 'MAINTENANCE' | 'OFFLINE' | 'NONE'; // What we will do locally
    reasoning: string;
    estimatedSavings: number; // in kWh
    preventedLoss: number; // in $
...
export const geminiService = {
    ...
    async evaluateAnomaly(
        ...
        try {
            ...
            // 1. Gather Context
            const recentLogs = await prisma.energyReading.findMany({
                where: { machineId },
            ...
            const decision: AIAutonomousDecision = JSON.parse(responseText);
            ...
            await prisma.alert.create({
                data: {
                    ...
                    severity: decision.riskLevel as 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL', // Map roughly
                }
            });
    ...
    async chat(message: string, context?: any) {
        try {
            ...
            const fallbackResponse = geminiService.generateFallbackResponse(message, context);
...
${machines.map((m: any) => `- **${m.name}** (${m.type}): ${m.status}`).join('\n')}
