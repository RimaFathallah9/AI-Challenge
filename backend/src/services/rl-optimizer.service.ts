...
export interface RLAnalysis {
    ...
    totalPotentialSavings: number; // kWh per day
    totalMoneySavings: number; // $ per day
...
export const rlOptimizerService = {
    /**
     ...
     */
    async analyzeAndOptimize(): Promise<RLAnalysis> {
        try {
            ...
            const totalReadings = await prisma.energyReading.count();
            const learningProgress = Math.min(100, (totalReadings / 5000) * 100);

            const analysis: RLAnalysis = {
