...
async function main() {
    ...
    // Create users
    const adminPw = await bcryptjs_1.default.hash('Admin1234!', 12);
    ...
    const managerPw = await bcryptjs_1.default.hash('Manager1234!', 12);
    await prisma.user.upsert({
    ...
    // Create machines
    const machineData = [
        { name: 'Hydraulic Press 45/TX-21', type: 'Hydraulic Press', status: client_1.MachineStatus.ONLINE },
        { name: 'CNC Machine Alpha-3', type: 'CNC Machine', status: client_1.MachineStatus.ONLINE },
        { name: 'Conveyor Belt B7', type: 'Conveyor Belt', status: client_1.MachineStatus.WARNING },
        { name: 'Air Compressor C12', type: 'Air Compressor', status: client_1.MachineStatus.ONLINE },
        { name: 'Welding Robot RW-9', type: 'Welding Robot', status: client_1.MachineStatus.MAINTENANCE },
        { name: 'CNC Machine Beta-7', type: 'CNC Machine', status: client_1.MachineStatus.ONLINE },
    ];
    const machines = [];
    ...
    // Seed sample alerts
    const alertMessages = [
        { message: 'High Consumption in Department B', severity: client_1.AlertSeverity.HIGH, details: 'Power draw exceeds normal threshold by 40%.' },
        { message: 'Machine X is operating at 80% efficiency. Check for potential issues.', severity: client_1.AlertSeverity.MEDIUM, details: 'Efficiency below optimal range.' },
        { message: 'Alarm Machine X performance dropping — currently at 60%, investigate possible causes', severity: client_1.AlertSeverity.CRITICAL, details: 'Machine X performance dropping — currently at 60% efficiency, which is below the critical threshold of 65%. Recommended Actions: Immediately check the machine electrical connections. Check lubrication levels. Ensure the machine is properly calibrated. Verify system parameters and update software if needed.' },
        { message: 'High Temperature warning on Air Compressor', severity: client_1.AlertSeverity.MEDIUM, details: 'Temperature exceeds safe operating limit.' },
    ];
