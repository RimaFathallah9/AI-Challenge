...
exports.digitalTwinService = {
     ...
     */
    async simulateProduction(input) {
        try {
            // ═══════════════════════════════════════════════════════════
            ...
            // ═══════════════════════════════════════════════════════════
            const historicalReadings = await client_1.prisma.energyReading.findMany({
            ...
            if (powerDeviation > 0.8) {
                console.warn(`⚠️ WARNING: Historical power range unusual for ${machine.type}. ` +
            ...
            if (!materialQuantity) {
                ...
                if (materialQuantity < minMaterial) {
                    throw new Error(`❌ Material quantity too low for target production. ` +
                ...
                if (materialQuantity > maxMaterial) {
                    throw new Error(`❌ Material quantity too high for target production. ` +
            ...
            if (optimizationFactor < 0.7 || optimizationFactor > 1.0) {
                console.warn(`⚠️ Optimization factor unusual: ${(optimizationFactor * 100).toFixed(1)}%. ` +
            ...
            // ═══════════════════════════════════════════════════════════
            const failureProbability = this.calculateFailureProbability(avgHistoricalTemp, avgHistoricalVibration, scaledPowerPerHour);
            const qualityImpact = this.assessQualityImpact(optimizedWasteRate, input.targetProduction);
            // ═══════════════════════════════════════════════════════════
            // STEP 13: GENERATE RECOMMENDATIONS
            // ═══════════════════════════════════════════════════════════
            const recommendations = this.generateProductionRecommendations(machine.type, avgHistoricalTemp, avgHistoricalVibration, optimizedWasteRate, moneySavings, input.operationHours);
            ...
            return {
                ...
                riskAssessment: {
                    ...
                    qualityImpact,
                    recommendation: failureProbability > 0.6
     ...
     */
    calculateMachineEfficiency(temperature, vibration) {
        let efficiency = 0.9;
        if (temperature > 80) {
            efficiency -= 0.25;
        }
     ...
     */
    calculateFailureProbability(temperature, vibration, power) {
        let probability = 0;
        if (temperature > 85) {
            probability += 0.35;
        }
     ...
     */
    assessQualityImpact(wasteRate, productionVolume) {
        if (wasteRate > 0.15) {
            return 'HIGH';
        }
     ...
     */
    generateProductionRecommendations(machineType, temperature, vibration, wasteRate, moneySavings, operationHours) {
     ...
     */
    async getProductionBaseline(machineId) {
        try {
     ...
     */
    generateRecommendations(reading, prediction) {
        const recommendations = [];
     ...
     * Simulate scenario
     */
    async simulateScenario(machineId, scenario, duration) {
        try {
            const machine = await client_1.prisma.machine.findUnique({
                where: { id: machineId },
            });
            ...
            let scenarioDisplayName = scenario;
            let lifespan = 10000;
            switch (scenario) {
                case 'NORMAL':
            ...
            const recommendation = failurePrediction.riskLevel === 'CRITICAL'
     ...
     * Test all scenarios
     */
    async testAllScenarios(machineId) {
        const results = [];
        const scenarios = [
            'NORMAL',
            'HEAVY_LOAD',
