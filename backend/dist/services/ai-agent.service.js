...
exports.aiAgentService = {
    ...
    async monitorMachine(machineId) {
        try {
            const machine = await client_1.prisma.machine.findUnique({
                where: { id: machineId },
            });
            if (!machine)
     ...
     */
    async detectAnomalies(machineId) {
        try {
            const readings = await client_1.prisma.energyReading.findMany({
                where: { machineId },
                orderBy: { timestamp: 'desc' },
    ...
    async predictMaintenance(machineId) {
        try {
            ...
            if (failureProbability > 0.3 && failureProbability <= 0.6)
                maintenanceType = 'PREDICTIVE';
            // Priority
            let priority = 'LOW';
            if (estimatedRUL < 7)
                priority = 'URGENT';
            else if (estimatedRUL < 30)
                priority = 'HIGH';
            else if (estimatedRUL < 90)
                priority = 'MEDIUM';
            return {
                machineId,
     ...
     */
    async optimizeEnergy(machineId) {
        try {
            const readings = await client_1.prisma.energyReading.findMany({
                where: { machineId },
                orderBy: { timestamp: 'desc' },
     ...
     */
    async makeAutonomousDecision(machineId) {
        try {
            ...
            if (currentMetrics.temperature > 90) {
                ...
                if (currentMetrics.temperature > 75)
                    decision = 'ACTIVATE_COOLING';
                if (currentMetrics.vibration > 3.5)
                    decision = 'REDUCE_LOAD';
                confidence = 0.7;
            ...
            const safetyMargin = Math.max(0, Math.min(1, (90 - currentMetrics.temperature) / 20, // Temp margin
     ...
     */
    async generateExplainableReport(machineId) {
        try {
            // Gather all intelligence
            const monitoring = await this.monitorMachine(machineId);
            const anomalies = await this.detectAnomalies(machineId);
            ...
            // Identify key issues
            const keyIssues = [];
            if (monitoring.state === 'CRITICAL')
                keyIssues.push('🚨 Machine in CRITICAL state');
            if (anomalies.length > 0) {
            ...
            // Recommended actions
            const recommendedActions = [decision.reasoning.explanation];
            ...
            try {
                ...
                await gemini_service_1.geminiService.chat(`Analyze machine ${machineId} health: Temperature ${monitoring.currentMetrics.temperature}°C, Vibration ${monitoring.currentMetrics.vibration}mm/s, Failure risk ${(maintenance.failureProbability * 100).toFixed(0)}%.`);
            }
            catch (e) {
     ...
     */
    async recordDecisionFeedback(machineId, decision, outcome, improvement) {
        try {
     ...
     */
    async getLearningHistory(machineId, limit = 50) {
        try {
     ...
     */
    async fuseDataSources(machineId) {
        try {
            // Get latest IoT readings
            const iotData = await client_1.prisma.energyReading.findFirst({
                where: { machineId },
                orderBy: { timestamp: 'desc' },
            });
            if (!iotData)
                throw new Error('No IoT data available');
            // Get production metrics
            const productionData = await client_1.prisma.energyReading.findMany({
                where: { machineId },
                orderBy: { timestamp: 'desc' },
     ...
     */
    async getAgentStatus(factoryId) {
        try {
            const machines = await client_1.prisma.machine.findMany({
                where: factoryId ? { factoryId } : undefined,
            });
            const reports = await Promise.all(machines.map(m => this.generateExplainableReport(m.id).catch(err => {
