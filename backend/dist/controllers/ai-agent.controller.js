...
exports.aiAgentController = {
     ...
     */
    async monitorMachine(req, res) {
        try {
            const { machineId } = req.params;
            const monitoring = await ai_agent_service_1.aiAgentService.monitorMachine(machineId);
            res.json({
                success: true,
                data: monitoring,
                timestamp: new Date(),
    ...
    async detectAnomalies(req, res) {
        try {
            const { machineId } = req.params;
            const anomalies = await ai_agent_service_1.aiAgentService.detectAnomalies(machineId);
            res.json({
                success: true,
    ...
    async predictMaintenance(req, res) {
        try {
            const { machineId } = req.params;
            const maintenance = await ai_agent_service_1.aiAgentService.predictMaintenance(machineId);
            res.json({
                success: true,
                data: maintenance,
                timestamp: new Date(),
     ...
     */
    async optimizeEnergy(req, res) {
        try {
            const { machineId } = req.params;
            const optimization = await ai_agent_service_1.aiAgentService.optimizeEnergy(machineId);
            res.json({
                success: true,
    ...
    async makeDecision(req, res) {
        try {
            const { machineId } = req.params;
            const decision = await ai_agent_service_1.aiAgentService.makeAutonomousDecision(machineId);
            res.json({
                success: true,
    ...
    async generateReport(req, res) {
        try {
            const { machineId } = req.params;
            const report = await ai_agent_service_1.aiAgentService.generateExplainableReport(machineId);
            res.json({
                success: true,
    ...
    async recordFeedback(req, res) {
        try {
            ...
            const learning = await ai_agent_service_1.aiAgentService.recordDecisionFeedback(machineId, decision, outcome, improvement);
            res.json({
                success: true,
    ...
    async getLearningHistory(req, res) {
        try {
            ...
            const history = await ai_agent_service_1.aiAgentService.getLearningHistory(machineId, limit);
            res.json({
                success: true,
     ...
     */
    async fuseData(req, res) {
        try {
            const { machineId } = req.params;
            const fused = await ai_agent_service_1.aiAgentService.fuseDataSources(machineId);
            res.json({
                success: true,
                data: fused,
                timestamp: new Date(),
    ...
    async getStatus(req, res) {
        try {
            ...
            const reports = await ai_agent_service_1.aiAgentService.getAgentStatus(factoryId);
            // Summary statistics
    ...
    async getDashboard(req, res) {
        try {
            ...
            const [monitoring, anomalies, maintenance, energy, decision, report, fused] = await Promise.all([
                ai_agent_service_1.aiAgentService.monitorMachine(machineId),
                ai_agent_service_1.aiAgentService.detectAnomalies(machineId),
                ai_agent_service_1.aiAgentService.predictMaintenance(machineId),
                ai_agent_service_1.aiAgentService.optimizeEnergy(machineId),
                ai_agent_service_1.aiAgentService.makeAutonomousDecision(machineId),
                ai_agent_service_1.aiAgentService.generateExplainableReport(machineId),
                ai_agent_service_1.aiAgentService.fuseDataSources(machineId),
            ]);
            res.json({
                success: true,
                machineId,
